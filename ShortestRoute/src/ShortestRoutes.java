...
public class ShortestRoutes {
	...
	public ShortestRoutes(Graph graph, int source, int destination) {
		...
		for (int i = 0; i < graph.numberOfStops - 1; i++) {
			int vertex = minimumDistanceTo(distTo, visited);
			if (vertex < 0) {
				continue;
			}
			visited[vertex] = true;
	...
	public int minimumDistanceTo(double[] distTo, boolean[] visited) {
	...
	public void dijkstra(Edge edge) {
